...
if len(sys.argv)<2:
  ...
  exit()
who = sys.argv[1] if len(sys.argv)==3 else twilio['admin']
toWhom = []
for w in who.split(','):
...
for who in toWhom:
  ...
  if feedback:
    uri = 'https://api.twilio.com/2010-04-01/Accounts/'+twilio['accountSID']+'/Messages/'+message.sid+'.json'
    #print(message.sid,uri)
    rqst = requests.get(uri,auth=(twilio['accountSID'],twilio['authToken']))
